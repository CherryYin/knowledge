        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT 
                    ...
                    MAX(chunk_type) as format,
                    COUNT(*) as chunk_count,
                    COUNT(DISTINCT toc_node_id) as toc_nodes,
                    MAX(CAST(metadata_json->>'depth' AS INTEGER)) as toc_depth,
            ...
            return {
                ...
                "toc_nodes": row["toc_nodes"] or 0,
                "toc_depth": row["toc_depth"] or 0,
                "created_at": row["created_at"].isoformat() if row["created_at"] else "",
                "toc_outline": "",
            }

    async def get_toc(self, doc_id: str) -> Optional[dict]:
        """获取文档 TOC 结构"""
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            # 获取所有唯一的 TOC 节点
            rows = await conn.fetch("""
                SELECT DISTINCT ON (toc_node_id)
                    toc_node_id,
                    section_title,
                    section_path,
                    metadata_json->>'depth' as depth
                FROM doc_chunks
                WHERE doc_id = $1 AND toc_node_id != ''
                ORDER BY toc_node_id, page
            """, doc_id)
            ...
            nodes = {}
            for row in rows:
                node_id = row["toc_node_id"]
                nodes[node_id] = {
                    "id": node_id,
                    "title": row["section_title"] or f"Section {node_id}",
                    "level": int(row["depth"] or 1),
                    "node_type": "section",
                    "children": [],
                }
            ...
            # 生成 outline 文本
            def build_outline(nodes: list, indent: int = 0) -> str:
                lines = []
                for node in nodes:
                    lines.append("  " * indent + node["title"])
                    if node["children"]:
                        lines.extend(build_outline(node["children"], indent + 1).split("\n"))
                return "\n".join(lines)
            
            outline = build_outline(tree)
